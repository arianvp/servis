...
Sadly enough, this code will give us a compiler error. The problem is that in Idris, we cannot pattern match on values of \mintinline{idris}{Type}. 
...
over a universe similarly to how we did in the definition of \mintinline{idris}{Handler}.
\begin{minted}{idris}
...
interface HasDocs a where
  docs :: a -> String
\end{minted}
Now, we need to define an implementation for \mintinline{idris}{API} for this interface and we are done. To implement this, we will of course also needs to implement the interface for \mintinline{idris}{Handler}, \mintinline{idris}{PathPart} and \mintinline{idris}{Path}.

To get an idea on how this would loook like, we will show the implementation of \mintinline{idris}{Handler} and \mintinline{idris}{API}. The others are left as an exercise for the reader:
\begin{minted}{idris}
...
Instead of letting the application return a value of \mintinline{idris}{IO String}, we let it return a value of \mintinline{idris}{Maybe (IO String)}, where \mintinline{idris}{Just a} signals a route match, and \mintinline{idris}{Nothing} signals a route mistmatch such that we can try another route.
...
Defining this router consists of two cases: A \mintinline{idris}{GET} or a \mintinline{idris}{POST}.
...
does not return \mintinline{idris}{Nothing}:
\begin{minted}{idris}
( FromCapture capture
...
Now we only have to be concerned with \mintinline{idris}{Const} and \mintinline{idris}{Capture}. Because \mintinline{idris}{Capture} signifies a variable substitution, it will always conflict with a \mintinline{idris}{Const}. E.g.,  \texttt{/users/hey} conflicts with \texttt{/users/:var} because \texttt{:var} could be substituted by \texttt{hey}. It will also conflict with another \mintinline{idris}{Capture}.
...
The fact that we can not construct a value of \mintinline{idris}{DisjointPP (Const x) (Capture x y)} nor a \mintinline{idris}{DisjointPP (Capture x y) (Capture z f)} indicates that these will always overlap, which is the behaviour that we wanted to express.
...
We will now show how we would implement this for \mintinline{idris}{GET}:
\begin{minted}{idris}
  (Universe req, Universe res) => Universe (Handler req res) where
